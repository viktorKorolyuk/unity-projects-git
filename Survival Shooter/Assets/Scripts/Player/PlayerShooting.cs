...
public class PlayerShooting : MonoBehaviour
{
    ...
    void Shoot ()
    {
        ...
        shootRay.direction = transform.forward; //the local z axis

        if(Physics.Raycast (shootRay, out shootHit, range, shootableMask)) //if it hits somthing
        {
            EnemyHealth enemyHealth = shootHit.collider.GetComponent <EnemyHealth> (); //get the "victim"
            if(enemyHealth != null) //does it have an enemyhealth script?
            {
